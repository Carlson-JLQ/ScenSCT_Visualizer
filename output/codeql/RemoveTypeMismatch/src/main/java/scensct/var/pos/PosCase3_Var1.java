// Call removeFirstOccurrence on a java.util.Deque with an argument of a type that is incompatible with the deque's element type should be flagged as a violation.
package scensct.var.pos;

import java.util.ArrayDeque;
import java.util.Deque;
import java.util.LinkedList;

public class PosCase3_Var1 {
    public static void main(String[] args) {
        Deque<Boolean> boolDeque = new ArrayDeque<>();
        boolDeque.add(Boolean.TRUE);
        Long removalCandidate = Long.valueOf(42);
        // Type mismatch remains: Long vs Boolean
        boolDeque.removeFirstOccurrence(removalCandidate); // [REPORTED LINE]
    }
}