// A java.text.SimpleDateFormat constructor call with a string literal first argument whose value contains "%Y" but does not contain "%M" should not be flagged as suspicious date format pattern.
package scensct.var.neg;

import java.text.SimpleDateFormat;

public class NegCase4_Var3 {
    public static void main(String[] args) {
        // Variant 3: Intra-procedural refactoring - use a temporary variable with control flow that doesn't alter the string
        String pattern = null;
        if (args.length == 0) {
            pattern = "%Y-%m-%d";
        } else {
            pattern = "%Y-%m-%d"; // same literal, unreachable branch
        }
        SimpleDateFormat sdf = new SimpleDateFormat(pattern);
    }
}